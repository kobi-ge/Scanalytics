...
export default function UploadReceipt() {
    ...
    const handleUpload = async () => {
        ...
        if (!user?._id) return alert("User error");
        ...
        try {
            ...
            alert("Receipt uploaded successfully: " + response.data.message);
            setProcessing(true);
            navigate("/");
            ...
            alert("Error uploading to Ingestion server");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-xl mx-auto mt-10 bg-white rounded-[32px] shadow-2xl border border-gold/10 overflow-hidden">
            <div className="bg-navy p-10 flex flex-col items-center border-b border-gold/20">
                ...
                <h2 className="text-2xl font-black text-gold">Scan New Receipt</h2>
            ...
            <div className="p-10 space-y-8 text-center">
                ...
                >
                    {loading ? "Processing & Uploading..." : "Upload for Processing"}
                </button>
            </div>
        </div>
    );
